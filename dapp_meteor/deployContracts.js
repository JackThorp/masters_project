...
CM = {
  ...
  deployAll: function() {
    ...
    var contractNames   = names.length > 0 ? names : CM.getContractNames(CONTRACTS_PATH);
    ...
    for(var i = 0; i < contractNames.length; i++) {
      ...
      var compiled = CM.compileContract(name, CONTRACTS_PATH);
      console.log("Compiled " + name);
      var jsModule = CM.writeJsModule(name, compiled);
      CM.writeToFile(jsModule, path.join('app', 'imports', 'contracts', name + '.js'));
    ...
    // in the system.
    CM.deployContract("CMC", compiledContracts["CMC"], txObj).then(function(cmc) {
      
      CMCContract = Promise.promisifyAll(cmc);

      _.forEach(compiledContracts, function(contract, name) {
        if (_.indexOf(toDeploy, name) >= 0) {
          deployedContracts.push(CM.deployContract(name, contract, txObj));
        }
      });
    ...
    .then(function(dcs) {
      ...
      CM.writeToFile(file, path.join('app', 'imports', 'startup', 'contractLocations.js'));
    })
    .catch(function(err) {
      console.log(err);
    });
  ...
  compileContract: function(name, contractsPath) {
  ...
  getContractNames: function(contractsPath) {
  ...
  writeJsModule: function(name, contract) {
  ...
  writeToFile: function(jsModule, path) {
  ...
  deployContract: function(name, contract, txObj) { 
